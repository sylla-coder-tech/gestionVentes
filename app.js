document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadDashboard() {
    ...
    rows.forEach(v => {
      ...
      totalPrixAchat += pa;        // KPI revenus = somme prix d'achat (comme tu veux)
      totalCaisse += (pa - liv);   // KPI caisse
    });
    ...
    if (!rows.length) {
      dashSalesBody.innerHTML = `<tr><td colspan="11">Aucune vente.</td></tr>`;
      return;
    }
  ...
  async function loadSales() {
    ...
    if (!data || !data.length) {
      salesBody.innerHTML = `<tr><td colspan="12">Aucune vente.</td></tr>`;
      return;
    }
